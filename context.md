...
### Giai đoạn 4 — Keyset Pagination & Scroll Preservation (GĐ4.2) ✅ HOÀN TẤT

- **Phân trang Keyset theo ID**: Chuyển đổi từ offset-based sang keyset-based (`beforeId`) tránh lệch tin hoặc trùng lặp khi có tin nhắn mới liên tục. Bỏ qua các ID tin nhắn AI tạm thời (giá trị âm) khi xác định ID tin nhắn cũ nhất.
- **Giữ vị trí cuộn (Scroll Preservation)**: Sử dụng `flushSync` từ `react-dom` để ép React cập nhật DOM đồng bộ trước khi đo và bù trừ `scrollTop` của container, tránh hiện tượng giật màn hình khi tải tin cũ.
- **Tự động tải tin cũ (Intersection Observer)**: Sử dụng API `IntersectionObserver` lắng nghe Sentinel Div ở đầu danh sách tin nhắn để tự động kích hoạt tải thêm lịch sử, đi kèm nút bấm liên kết thủ công làm fallback và hiển thị thông báo "Đầu hội thoại" khi đã tải hết.
