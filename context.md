...
- **GĐ5.5 Gửi Ảnh & File Đính Kèm (HOÀN TẤT)**:
  - **DB & Model**: 4 cột `attachment_url`, `attachment_name`, `attachment_type`, `attachment_size` trong `DbInitializer` & `Message` model + cập nhật toàn bộ SQL SELECT/INSERT trong `MessageRepository`.
  - **Threat-Model Security**: `UploadController.cs` `POST /api/upload` `[Authorize]` thực thi đủ 5 quy tắc (giới hạn ≤10MB, whitelist extension `jpg,png,gif,webp,pdf` + **tuyệt đối cấm SVG**, đổi tên `{guid}{ext}`, sanitize tên gốc). `Program.cs` cấu hình `UseStaticFiles` với header `nosniff` & `Content-Disposition: attachment` cho file tài liệu.
  - **SignalR & Frontend**: `SendMessage`/`SendPrivateMessage` nhận tham số optional đính kèm. `ChatView` nút kẹp giấy chọn file, validate client, hiển thị thanh xem trước đính kèm. `MessageBubble` render ảnh inline với AntD `<Image preview>` hoặc chip đính kèm `"📎 tên_file (dung_lượng)"` bấm tải về.
