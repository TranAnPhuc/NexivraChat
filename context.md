...
- **Tối ưu giao diện Mobile (Responsive Layout) (HOÀN TẤT)**:
  - **Hook `useIsMobile`**: Tạo hook lắng nghe sự kiện `resize` thông qua `window.matchMedia` kết hợp listener để cập nhật trạng thái mobile động dưới `768px`.
  - **Layout trượt (Drawer)**: Tách biệt layout desktop và mobile để loại bỏ hoàn toàn rủi ro regression. Trên mobile, `RoomSidebar` trượt từ bên trái và `CopilotPanel` trượt từ bên phải dưới dạng Ant Design `<Drawer>`. Khi chọn phòng/user hoặc gửi lệnh cho AI, các drawer tự động đóng lại để trả lại không gian tối đa.
  - **Hamburger Menu & AI Toggle**: Header được tái thiết kế tự động hiển thị nút ☰ (Menu) bên trái và nút AI bên phải trên mobile, đồng thời tự động cắt ngắn/ellipsis tên phòng quá dài để không làm vỡ bố cục.
  - **Dynamic Viewport Height (`100dvh`)**: Khắc phục triệt để lỗi bị thanh địa chỉ hoặc bàn phím ảo của trình duyệt di động che mất phần nhập liệu bằng cách chuyển đổi `height: 100vh` thành `100dvh`.
  - **Chống Auto-Zoom trên iOS**: Tăng cỡ chữ (`fontSize`) của ô nhập liệu lên `16px` khi ở mobile, ngăn chặn cơ chế tự động phóng to phiền toái của iOS khi người dùng click vào ô chat.
  - **Clamp & Responsive views**: Giới hạn bề ngang tối đa của panel absolute tìm kiếm (`min(320px, calc(100vw - 24px))`) và dropdown Mention (`min(220px, calc(100vw - 32px))`). LoginView và ProfileView được thiết kế lại theo cột dọc (`flex-direction: column`) co giãn mượt mà trên màn hình nhỏ.
- **Tối ưu trải nghiệm & Độ tin cậy chat (UX & Reliability) (HOÀN TẤT)**:
  - **Trạng thái kết nối (Connection Status Banner)**: Lắng nghe SignalR connection events (`onreconnecting`, `onreconnected`, `onclose`) và render banner cảnh báo mỏng ngay dưới header: "Đang kết nối..." (lần đầu, cold-start), "Đang kết nối lại...", hoặc "Mất kết nối — đang thử lại" tùy thuộc trạng thái. Sử dụng deduplication key (`conn-error`) để gom nhóm các lỗi kết nối lặp đi lặp lại.
  - **Optimistic Send & Reconciliation**: Sinh `clientId` (UUID) và cập nhật danh sách tin nhắn ngay tức thì với trạng thái `sending` và opacity mờ (`0.6`). Sau khi backend dội lại tin nhắn thật kèm `clientId`, client thực hiện reconcile (thay thế) tin nhắn tạm bằng tin nhắn chính thức từ DB $\rightarrow$ loại bỏ hoàn toàn hiện tượng trùng lặp tin nhắn.
  - **Bong bóng lỗi & Retry**: Nếu Hub invoke lỗi, tin nhắn tạm chuyển sang trạng thái `failed` (hiển thị viền/chữ đỏ kèm nhãn ⚠️ Gửi lỗi và nút "Gửi lại"). Khi click "Gửi lại", client tự động kích hoạt gửi lại tin nhắn đó với cùng `clientId` và nội dung nguyên bản.
  - **Loading lịch sử lần đầu**: Bổ sung cờ `loadingHistory` điều khiển hiển thị vòng xoay `<Spin>` khi tải lịch sử phòng/DM lần đầu, giải quyết triệt để lỗi nhấp nháy dòng chữ empty-state *"Chưa có tin nhắn"* trước khi tin nhắn tải xong.
